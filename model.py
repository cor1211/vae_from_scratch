...
def transposed_conv(in_conv:int, out_conv:int, kernel_size:int, stride: int, padding:int, relu:bool = False, unpool: bool = False, norm:str = ''):
    layer = []
    layer.append(nn.ConvTranspose2d(in_channels=in_conv, out_channels=out_conv, kernel_size=kernel_size, padding=padding, stride=stride))
    if norm:
...
class Encoder(nn.Module):
    def __init__(self, in_channels, dim_latent, device):
        ...
        # Layer 1
        self.layer_1 = conv2d(in_conv=in_channels, out_conv=8, kernel_size=5, relu=True, max_pool=True, norm='bn')
        # self.bn_1 = get_norm_layer(dim=2, norm='bn', out_channel=8)
        # layer 2 
        self.layer_2 = conv2d(in_conv=8, out_conv=16, kernel_size=3, relu=True, max_pool=False, norm='bn')
        # self.bn_2 = get_norm_layer(dim=2, norm='bn', out_channel=16)
        # layer 3
        self.layer_3 = conv2d(in_conv=16, out_conv=32, kernel_size=3, relu=True, max_pool=True, norm='bn')
        # self.bn_3 = get_norm_layer(dim=2, norm='bn', out_channel=32)
        # layer 4
        self.layer_4 = conv2d(in_conv=32, out_conv=64, kernel_size=3, relu=True, max_pool=False, norm='bn')
        # self.bn_4 = get_norm_layer(dim=2, norm='bn', out_channel=64)
        # layer 5
        self.layer_5 = linear(in_features=32*32*64, out_features=4096, relu=True, norm='bn')
    ...
    def forward(self, x):
        ...
        mean, logvar = torch.chunk(input = x, chunks=2, dim=1)
...
class Decoder(nn.Module):
    def __init__(self, dim_latent, out_channels):
        super().__init__()
        ...
        self.layer_4d = transposed_conv(in_conv=4, out_conv=32, kernel_size=4, padding=1, stride = 2, relu=True, norm='bn')
        # self.unpool_4d = nn.MaxUnpool2d(kernel_size=2, stride=2)
        # self.bn_4 = get_norm_layer(2, norm='bn', out_channel=32)

        self.layer_3d = transposed_conv(in_conv=32, out_conv=16, kernel_size=3, padding=1, stride = 1, relu=True, norm='bn')

        self.layer_2d = transposed_conv(in_conv=16, out_conv=8, kernel_size=4, padding=1, stride=2, relu=True, norm='bn')
        # self.unpool_2d = nn.MaxUnpool2d(kernel_size=2, stride=2)
        # self.bn_2 = get_norm_layer(dim=2, norm='bn', out_channel=8)

        self.layer_1d = transposed_conv(in_conv=8, out_conv= out_channels, kernel_size=5, padding=2, stride=1, norm='bn')
...
class VAE(nn.Module):                                                                                             
    def __init__(self, in_channels, out_channels, dim_latent, device):
        ...
        self.decoder = Decoder(dim_latent=dim_latent, out_channels=out_channels)

    def forward(self, x):
        mean, logvar, x = self.encoder(x)
